...
let (dryOut, dryRc) = runIn(testDir, "ustva --test -c " & ustvaConf & " --period 41 --dry-run")
...
let (prodOut, prodRc) = runIn(testDir, "ustva -c " & ustvaConf & " --period 41 --dry-run")
check("production dry-run exits 0", prodRc == 0, prodOut)
check("production no Testmerker", not prodOut.contains("Testmerker"))
let (testOut, testRc) = runIn(testDir, "ustva --test -c " & ustvaConf & " --period 41 --dry-run")
...
let (dryBoth, dryBothRc) = runIn(testDir, "ustva -c " & ustvaConf & " --period 01 --dry-run")
...
echo "--- ustva: 0% Kz45 ---"
writeFile(ustvaTsv, "1000,19\n500,0\n")
let (kz45Out, kz45Rc) = runIn(testDir, "ustva -c " & ustvaConf & " --period 01 --dry-run")
...
writeFile(ustvaTsv, "1000,19\n-200,19\n")
let (negOut, negRc) = runIn(testDir, "ustva -c " & ustvaConf & " --period 01 --dry-run")
...
writeFile(ustvaTsv, "1000,19%\n500,7%\n")
let (pctOut, pctRc) = runIn(testDir, "ustva -c " & ustvaConf & " --period 01 --dry-run")
...
writeFile(ustvaTsv, "")
let (emptyOut, emptyRc) = runIn(testDir, "ustva -c " & ustvaConf & " --period 01 --dry-run")
...
let (pq1, _) = runIn(testDir, "ustva -c " & ustvaConf & " --period q1 --dry-run")
check("q1 -> 41", pq1.contains("<Zeitraum>41</Zeitraum>"))
let (pmar, _) = runIn(testDir, "ustva -c " & ustvaConf & " --period mar --dry-run")
check("mar -> 03", pmar.contains("<Zeitraum>03</Zeitraum>"))
let (p3, _) = runIn(testDir, "ustva -c " & ustvaConf & " --period 3 --dry-run")
...
euer = freiberuf.tsv
""")
  let (yOut, yRc) = runIn(testDir, "ustva -c " & yearConf & " --period 41 --dry-run")
...
versteuerung = ist
""")
let (badOut, badRc) = runIn(testDir, "ustva -c " & badConf & " --period q1 --dry-run")
...
let (ambigOut, ambigRc) = runIn(testDir, "ustva -c " & multiConf & " --period 41 --dry-run")
...
let (gOut, gRc) = runIn(testDir, "ustva mygewerbe -c " & multiConf & " --period 41 --dry-run")
...
let (fOut, _) = runIn(testDir, "ustva freiberuf -c " & multiConf & " --period 41 --dry-run")
check("other source uses personal taxnumber", fOut.contains("<Steuernummer>9198011310010</Steuernummer>"))

let (uOut, uRc) = runIn(testDir, "ustva bogus -c " & multiConf & " --period 41 --dry-run")
...
let (eDryOut, eDryRc) = runIn(testDir, "euer freelance -c " & euerConf & " --dry-run")
...
writeFile(euerTsv, "1000,19\n-300,19\n")
let (splitOut, splitRc) = runIn(testDir, "euer freelance -c " & euerConf & " --dry-run")
...
removeFile(euerTsv)
let (missOut, missRc) = runIn(testDir, "euer freelance -c " & euerConf & " --dry-run")
...
versteuerung = 2
""")
let (noEOut, noERc) = runIn(testDir, "euer freelance -c " & noEuerConf & " --dry-run")
...
let (noUstOut, noUstRc) = runIn(testDir, "ust freelance -c " & noEuerConf & " --dry-run")
check("ust unset ok", structuralOk(noUstOut, noUstRc), noUstOut)
check("ust unset warns", noUstOut.contains("Warning"))

let (noUvOut, noUvRc) = runIn(testDir, "ustva freelance -c " & noEuerConf & " --period 41 --dry-run")
...
euer = freelance.tsv
""")
  let (yOut, yRc) = runIn(testDir, "euer freelance -c " & yearConf & " --dry-run")
...
let (estOut, estRc) = runIn(testDir, "est --test -c " & estConf & " --dry-run --force")
...
let (estSOut, estSRc) = runIn(testDir, "est -c " & estConfS & " --dry-run --force")
...
let (mOut, mRc) = runIn(testDir, "est -c " & estMultiConf & " --dry-run --force")
...
let (nsOut, nsRc) = runIn(testDir, "est -c " & estNoSrc & " --dry-run --force")
...
let (dedOut, dedRc) = runIn(testDir, "est -c " & estDedConf & " --dry-run")
...
euer = mybiz.tsv
""")
let (noDedOut, _) = runIn(testDir, "est -c " & estNoDedConf & " --dry-run")
check("missing deductions warns", noDedOut.contains("Warning") and noDedOut.contains("deductions"))
let (forceOut, forceRc) = runIn(testDir, "est -c " & estNoDedConf & " --dry-run --force")
...
""")
let (kapOut, kapRc) = runIn(testDir, "est -c " & estKapConf & " --dry-run --force")
...
writeFile(testDir / "tmp_kind_ded.tsv",
  "code\tamount\nmax174\t2400\nlisa174\t3600\nlisa176\t1500\n")
let (kindOut, kindRc) = runIn(testDir, "est -c " & estKindConf & " --dry-run")
...
""")
let (andPOut, andPRc) = runIn(testDir, "est -c " & estAndPConf & " --dry-run --force")
...
euer = mybiz.tsv
""")
  let (yOut, yRc) = runIn(testDir, "est -c " & yConf & " --dry-run --force")
...
let (uDryOut, uDryRc) = runIn(testDir, "ust freelance --test -c " & ustConf & " --dry-run")
...
writeFile(ustTsv, "1000,19\n-300,19\n")
let (uspOut, uspRc) = runIn(testDir, "ust freelance -c " & ustConf & " --dry-run")
...
""")
writeFile(ustTsv, "1000,19\n")
let (vzOut, vzRc) = runIn(testDir, "ust freelance -c " & ustVzConf & " --dry-run")
...
euer = freelance.tsv
""")
  let (yOut, yRc) = runIn(testDir, "ust freelance -c " & yConf & " --dry-run")
...
let (mDryOut, mDryRc) = runIn(testDir, "message --test -c " & messageConf &
  " --subject \"Test Betreff\" --text \"Test Nachricht\" --dry-run")
...
let (iOut, iRc) = runIn(testDir, "iban --test -c " & ibanConf &
  " --new-iban DE89370400440532013000 --dry-run")
...
echo "--- list --dry-run ---"
let (lOut, lRc) = runIn(testDir, "list -c " & abholConf & " --dry-run")
...
echo "--- download --dry-run ---"
let (dOut, dRc) = runIn(testDir, "download -c " & abholConf & " --dry-run")
...
let (genEstOut, genEstRc) = run(Viking & " est -c " & (initDir / "viking.conf") & " --dry-run --force")
...
writeConf(noYearConf,
  ...
  "religion = 11\n" &
  "beruf = Software-Entwickler\n")
let (nyOut, nyRc) = runIn(testDir, "est -c " & noYearConf & " --dry-run --force")
